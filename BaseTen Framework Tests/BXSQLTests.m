...
- (void) testDisableEnable
{
	...
	XCTAssertTrue ([self checkEnablingForTest: connection]);
	...
	XCTAssertFalse ([self checkEnablingForTest: connection]);
...
- (void) testPrune
{
	...
	XCTAssertTrue ([res querySucceeded],@"%@", [[res error] description]);
	[res advanceRow];
	XCTAssertTrue (0 == [[res valueForKey: @"count"] integerValue]);

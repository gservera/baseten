...
- (void) testPeopleDepartments
{
	...
	NSArray* expected = [NSArray arrayWithObjects:
						 ...
						 @"CREATE TABLE \"test_schema\".\"Person\" (id SERIAL, \"birthday\" timestamp with time zone , \"surname\" text ) ;",
						 ...
						 nil];
	XCTAssertEqualObjects (expected, statements);
}
...
- (void) testRelationshipOptionality
{
	...
	XCTAssertTrue (38 == [statements count]);
	NSArray* creationStatements = [statements subarrayWithRange: NSMakeRange (1, 20)];
	NSArray* constraintStatements = [statements subarrayWithRange: NSMakeRange (21, 16)];
	
	XCTAssertEqualObjects ([statements objectAtIndex: 0], @"CREATE SCHEMA \"test_schema\";");
	
	{
		...
		while ((stmt = [e nextObject]))
		{
			...
			XCTAssertFalse (NSNotFound == i);
			
			stmt = [e nextObject];
			NSUInteger j = [creationStatements indexOfObject: stmt];
			XCTAssertFalse (NSNotFound == j);
			XCTAssertTrue (i + 1 == j);
		}
	}
	
	{
		...
		BXEnumerate (expectedConstraintStmts, e, [constraintStmtArrays objectEnumerator])
		{
			...
			XCTAssertFalse (NSNotFound == addColIdx);
			...
			while ((stmt = [e nextObject]))
			{
				...
				XCTAssertFalse (NSNotFound == idx);
				XCTAssertTrue (addColIdx < idx);
			}
		}
	}

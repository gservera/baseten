...
CREATE FUNCTION "baseten".eq (point, point) RETURNS BOOLEAN AS $$
	SELECT $1 ~= $2;
$$ IMMUTABLE STRICT LANGUAGE SQL;
REVOKE ALL PRIVILEGES ON FUNCTION "baseten".eq (point, point) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION "baseten".eq (point, point) TO basetenread
;

CREATE FUNCTION "baseten".eq (box, box) RETURNS BOOLEAN AS $$
	SELECT $1 ~= $2;
$$ IMMUTABLE STRICT LANGUAGE SQL;
REVOKE ALL PRIVILEGES ON FUNCTION "baseten".eq (box, box) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION "baseten".eq (box, box) TO basetenread
;

CREATE FUNCTION "baseten".eq (polygon, polygon) RETURNS BOOLEAN AS $$
	SELECT $1 ~= $2;
$$ IMMUTABLE STRICT LANGUAGE SQL;
REVOKE ALL PRIVILEGES ON FUNCTION "baseten".eq (polygon, polygon) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION "baseten".eq (polygon, polygon) TO basetenread
;

CREATE FUNCTION "baseten".eq (circle, circle) RETURNS BOOLEAN AS $$
	SELECT $1 ~= $2;
$$ IMMUTABLE STRICT LANGUAGE SQL;
REVOKE ALL PRIVILEGES ON FUNCTION "baseten".eq (circle, circle) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION "baseten".eq (circle, circle) TO basetenread
;
...
CREATE FUNCTION "baseten".same (anynonarray, anynonarray) RETURNS BOOLEAN AS $$
	SELECT (
...
REVOKE ALL PRIVILEGES ON FUNCTION "baseten".same (anynonarray, anynonarray) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION "baseten".same (anynonarray, anynonarray) TO basetenread
;

CREATE FUNCTION "baseten".same (anyarray, anyarray) RETURNS BOOLEAN AS $$
	SELECT (
		$1 IS NULL AND $2 IS NULL
	) OR (
		$1 IS NOT NULL AND
...
$$ IMMUTABLE CALLED ON NULL INPUT LANGUAGE SQL;
REVOKE ALL PRIVILEGES ON FUNCTION "baseten".same (anyarray, anyarray) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION "baseten".same (anyarray, anyarray) TO basetenread
;



CREATE FUNCTION "baseten".between (SMALLINT [2], SMALLINT) RETURNS BOOLEAN AS $$
	SELECT $1[1] <= $2 AND $2 <= $1[2];
$$ IMMUTABLE STRICT LANGUAGE SQL;
REVOKE ALL PRIVILEGES ON  FUNCTION "baseten".between (SMALLINT [2], SMALLINT) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION "baseten".between (SMALLINT [2], SMALLINT) TO basetenread;
...
CREATE FUNCTION "baseten".between (INTEGER [2], INTEGER) RETURNS BOOLEAN AS $$
	SELECT $1[1] <= $2 AND $2 <= $1[2];
$$ IMMUTABLE STRICT LANGUAGE SQL;
REVOKE ALL PRIVILEGES ON  FUNCTION "baseten".between (INTEGER [2], INTEGER) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION "baseten".between (INTEGER [2], INTEGER) TO basetenread;
...
CREATE FUNCTION "baseten".between (BIGINT [2], BIGINT) RETURNS BOOLEAN AS $$
	SELECT $1[1] <= $2 AND $2 <= $1[2];
$$ IMMUTABLE STRICT LANGUAGE SQL;
REVOKE ALL PRIVILEGES ON  FUNCTION "baseten".between (BIGINT [2], BIGINT) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION "baseten".between (BIGINT [2], BIGINT) TO basetenread;
...
CREATE FUNCTION "baseten".between (NUMERIC [2], NUMERIC) RETURNS BOOLEAN AS $$
	SELECT $1[1] <= $2 AND $2 <= $1[2];
$$ IMMUTABLE STRICT LANGUAGE SQL;
REVOKE ALL PRIVILEGES ON  FUNCTION "baseten".between (NUMERIC [2], NUMERIC) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION "baseten".between (NUMERIC [2], NUMERIC) TO basetenread;
...
CREATE FUNCTION "baseten".between (REAL [2], REAL) RETURNS BOOLEAN AS $$
	SELECT $1[1] <= $2 AND $2 <= $1[2];
$$ IMMUTABLE STRICT LANGUAGE SQL;
REVOKE ALL PRIVILEGES ON  FUNCTION "baseten".between (REAL [2], REAL) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION "baseten".between (REAL [2], REAL) TO basetenread;
...
CREATE FUNCTION "baseten".between (DOUBLE PRECISION [2], DOUBLE PRECISION) RETURNS BOOLEAN AS $$
	SELECT $1[1] <= $2 AND $2 <= $1[2];
$$ IMMUTABLE STRICT LANGUAGE SQL;
REVOKE ALL PRIVILEGES ON  FUNCTION "baseten".between (DOUBLE PRECISION [2], DOUBLE PRECISION) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION "baseten".between (DOUBLE PRECISION [2], DOUBLE PRECISION) TO basetenread;

CREATE OPERATOR "baseten".<<>> (
	PROCEDURE = "baseten".between,
	LEFTARG = DOUBLE PRECISION[2],
	RIGHTARG = DOUBLE PRECISION
);

CREATE FUNCTION "baseten".between (TIMESTAMP WITHOUT TIME ZONE [2], TIMESTAMP WITHOUT TIME ZONE) RETURNS BOOLEAN AS $$
	SELECT $1[1] <= $2 AND $2 <= $1[2];
$$ IMMUTABLE STRICT LANGUAGE SQL;
REVOKE ALL PRIVILEGES ON  FUNCTION "baseten".between (TIMESTAMP WITHOUT TIME ZONE [2], TIMESTAMP WITHOUT TIME ZONE) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION "baseten".between (TIMESTAMP WITHOUT TIME ZONE [2], TIMESTAMP WITHOUT TIME ZONE) TO basetenread;

CREATE OPERATOR "baseten".<<>> (
	PROCEDURE = "baseten".between,
	LEFTARG = TIMESTAMP WITHOUT TIME ZONE[2],
	RIGHTARG = TIMESTAMP WITHOUT TIME ZONE
);

CREATE FUNCTION "baseten".between (TIMESTAMP WITH TIME ZONE [2], TIMESTAMP WITH TIME ZONE) RETURNS BOOLEAN AS $$
	SELECT $1[1] <= $2 AND $2 <= $1[2];
$$ IMMUTABLE STRICT LANGUAGE SQL;
REVOKE ALL PRIVILEGES ON  FUNCTION "baseten".between (TIMESTAMP WITH TIME ZONE [2], TIMESTAMP WITH TIME ZONE) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION "baseten".between (TIMESTAMP WITH TIME ZONE [2], TIMESTAMP WITH TIME ZONE) TO basetenread;

CREATE OPERATOR "baseten".<<>> (
	PROCEDURE = "baseten".between,
	LEFTARG = TIMESTAMP WITH TIME ZONE[2],
	RIGHTARG = TIMESTAMP WITH TIME ZONE
);

CREATE FUNCTION "baseten".between (INTERVAL [2], INTERVAL) RETURNS BOOLEAN AS $$
	SELECT $1[1] <= $2 AND $2 <= $1[2];
$$ IMMUTABLE STRICT LANGUAGE SQL;
REVOKE ALL PRIVILEGES ON  FUNCTION "baseten".between (INTERVAL [2], INTERVAL) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION "baseten".between (INTERVAL [2], INTERVAL) TO basetenread;
...
CREATE FUNCTION "baseten".between (DATE [2], DATE) RETURNS BOOLEAN AS $$
	SELECT $1[1] <= $2 AND $2 <= $1[2];
$$ IMMUTABLE STRICT LANGUAGE SQL;
REVOKE ALL PRIVILEGES ON  FUNCTION "baseten".between (DATE [2], DATE) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION "baseten".between (DATE [2], DATE) TO basetenread;
...
CREATE FUNCTION "baseten".between (TIME WITHOUT TIME ZONE [2], TIME WITHOUT TIME ZONE) RETURNS BOOLEAN AS $$
	SELECT $1[1] <= $2 AND $2 <= $1[2];
$$ IMMUTABLE STRICT LANGUAGE SQL;
REVOKE ALL PRIVILEGES ON  FUNCTION "baseten".between (TIME WITHOUT TIME ZONE [2], TIME WITHOUT TIME ZONE) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION "baseten".between (TIME WITHOUT TIME ZONE [2], TIME WITHOUT TIME ZONE) TO basetenread;

CREATE OPERATOR "baseten".<<>> (
	PROCEDURE = "baseten".between,
	LEFTARG = TIME WITHOUT TIME ZONE[2],
	RIGHTARG = TIME WITHOUT TIME ZONE
);

CREATE FUNCTION "baseten".between (TIME WITH TIME ZONE [2], TIME WITH TIME ZONE) RETURNS BOOLEAN AS $$
	SELECT $1[1] <= $2 AND $2 <= $1[2];
$$ IMMUTABLE STRICT LANGUAGE SQL;
REVOKE ALL PRIVILEGES ON  FUNCTION "baseten".between (TIME WITH TIME ZONE [2], TIME WITH TIME ZONE) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION "baseten".between (TIME WITH TIME ZONE [2], TIME WITH TIME ZONE) TO basetenread;

CREATE OPERATOR "baseten".<<>> (
	PROCEDURE = "baseten".between,
	LEFTARG = TIME WITH TIME ZONE[2],
